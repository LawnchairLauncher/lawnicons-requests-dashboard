...
const GOAL_START = 10942;
const TOP_COUNTRIES = ['bd', 'br', 'de', 'in', 'ru', 'uk', 'us'];
const COUNTRIES = {
  "ad": "Andorra",
    ...
    const tagHtml = tags.map((tagId) => {
      if (tagId === 'supported') {
        ...
        if (issueEntry) {
          ...
          const issueUrl = `https://github.com/LawnchairLauncher/lawnicons/issues/${issueNum}`;
  ...
  updateHeader() {
    ...
    if (App.state.lastUpdate) {
      ...
      const displayText = timeAgo === 'Today'
        ...
        : `Updated ${timeAgo} ago`;
      countEl.innerHTML =
        `<a href="https://github.com/LawnchairLauncher/lawnicons-requests-dashboard" target="_blank" title="Last update: ${fullDate}">${displayText}</a>`;
    } else {
      countEl.innerHTML = '';
    }
    ...
    if (headerRight) {
      ...
      if (!document.getElementById('guidesLink')) {
        headerRight.insertAdjacentHTML('afterbegin', `
          <a id="guidesLink" href="https://github.com/LawnchairLauncher/lawnicons/blob/develop/CONTRIBUTING.md#contributing-icons-tldr" class="header-link" title="Design guidelines" target="_blank" rel="noopener">
...
renderContributionMode() {
  ...
  renderIconLibrary() {
    ...
    if (title) {
      title.innerHTML = `Found in Lawnicons <span style="color: var(--on-surface-variant); margin-left: var(--space-xs)">${matches.length}</span>`;
    }
